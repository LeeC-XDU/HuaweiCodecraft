/***************************************************************
该文件需要给主函数参数，指定要测试的文件
语法：可执行程序 xxx/topo.csv xxx/demand.csv xxx/result.csv
***************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
bool detection(int *array, int arrayCount, const char *fileName);

int main(int argc, char *argv[])
{
	if(argc < 4)
	{
		printf("Usage: %s xxx/topo.csv xxx/demand.csv xxx/result.csv\n", argv[0]);
		exit(EXIT_FAILURE);
	}

	FILE * topocsv = fopen(argv[1], "r");
	FILE * resultcsv = fopen(argv[3], "r");
	...
	if(detection(node, nodeArrayCount, argv[2]))
		printf("OK\n");
	else
		printf("Error\n");
